...
######################################################################			Path_Following
def path_f(xd,yd,thetad,step,a,b):
	global current_x
	global current_y
	...
	file = open(save_folder2 + "Line"+"_vr_"+str(vr)+"_K_"+str(h)+"_delta_"+str(delta_min)+'_step_'+str(step)+".txt","a")
...
try: 
	while True:
		mode = str(input("Enter x: x_axis, y: y_axis"))
		...
		if mode == 'x':	
			...
			for i in x:
				...
				yd = 0
				thetad = 0
				path_f(float(xd),float(yd),float(thetad),float(step),a,b)
