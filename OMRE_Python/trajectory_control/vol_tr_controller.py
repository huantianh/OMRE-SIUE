	...
	while True:
		...
		if mode == 's':
			...
			while True:
				...
				j_dot = (2*np.pi*0.03/60)*np.array([(2/3)*np.cos(thetad+np.pi/3),(-2/3)*np.cos(thetad),(2/3)*np.cos(thetad-np.pi/3),(2/3)*np.sin(thetad+np.pi/3),(-2/3)*np.sin(thetad),(2/3)*np.sin(thetad-np.pi/3),0,0,0]).reshape(3,3)
